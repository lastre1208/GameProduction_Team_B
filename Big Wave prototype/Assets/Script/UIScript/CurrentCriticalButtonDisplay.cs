...
public class CurrentCriticalButtonDisplay : MonoBehaviour
{
    [Header("Aボタンのアイコン")]
    ...
    void DisplayButton()//ボタン表示
    {
        ...
        {
            Button currentCriricalButton = critical.CriticalButton[0];//現在の指定されているボタン
            //指定されているボタンを表示
            for (int i=0;i< Enum.GetNames(typeof(Button)).Length; i++)
            {
                if((Button)i==currentCriricalButton)
                {
